// $Id: ugen_UGen.h 1006 2010-03-18 12:26:09Z mgrobins $
// $HeadURL: http://dsrowlan@164.11.131.73/svn/CEMS/mt/other/UGenProject/UGen/core/ugen_UGen.h $
...
#define ReportSize(CLASSNAME) printf(#CLASSNAME " size = %d\n", sizeof(CLASSNAME))
...
public:
	...
	inline float* processBlock(bool& shouldDelete, const unsigned int blockID, const int channel = -1) throw()
	{ 	
	...
	float* prepareAndProcessBlock(const int actualBlockSize, const unsigned int blockID, const int channel = -1) throw();
	...
	inline void prepareForBlock(const int actualBlockSize, const unsigned int blockID) throw()
	{	
	...
	UGen& getInput(const int input, const int channel) throw();
	...
	inline static UGenInternal*		getNullInternal() throw()					{ return null.getInternalUGen(0);										}
	
	/** Get a control rate null internal. @return A control rate null internal. */
	inline static UGenInternal*		getNullInternalKr() throw()					{ return nullKr.getInternalUGen(0);										}
...
//		null = UGen();
//		nullKr = UGen().kr();
//		null.decrementInternals();
//		nullKr.decrementInternals();
#ifdef JUCE_VERSION
...
//			nullKr = UGen().kr();
			...
			Buffer::tableSine512  = Buffer::line( 513, 0.0, 2.0 * pi).sin().shrinkSize();
			Buffer::tableSine8192 = Buffer::line(8193, 0.0, 2.0 * pi).sin().shrinkSize();
			Buffer::tableCosine512  = Buffer::line( 513, 0.0, 2.0 * pi).cos().shrinkSize();
			Buffer::tableCosine8192 = Buffer::line(8193, 0.0, 2.0 * pi).cos().shrinkSize();
//			Buffer::tableSine512  = Buffer::line( 1025, 0.0, 4.0 * pi).sin().shrinkSize(512);
//			Buffer::tableSine8192 = Buffer::line(16385, 0.0, 4.0 * pi).sin().shrinkSize(8192);

			Buffer constantPanTemp;
			constantPanTemp = Buffer::line(512, -pi4, pi4) << Buffer::line(512, pi4, -pi4);
			Buffer::tableConstantPan512 = (constantPanTemp.cos() - constantPanTemp.sin()) * sqrt22;
			isInitialised = true;
