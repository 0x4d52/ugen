// $Id: ugen_PlayBuf.cpp 993 2010-03-03 21:47:50Z mgrobins $
// $HeadURL: http://dsrowlan@164.11.131.73/svn/CEMS/mt/other/UGenProject/UGen/buffers/ugen_PlayBuf.cpp $
...
PlayBufUGenInternal::PlayBufUGenInternal(Buffer const& buffer, UGen const& rate, UGen const& trig, UGen const& offset, UGen const& loop) throw()
...
{	
	return new PlayBufUGenInternal(buffer_.getChannel(channel), inputs[Rate], inputs[Trig], inputs[Offset], inputs[Loop]);
}
...
PlayBuf::PlayBuf(Buffer const& buffer, UGen const& rate, UGen const& trigger, UGen const& startPos, UGen const& loop) throw()
{	
	...
	if(numChannels > 0 && buffer.size() > 0)
	{
		...
		generateFromProxyOwner(new PlayBufUGenInternal(buffer, rate.mix(), trigger.mix(), startPosChecked, loop.mix()));
