// $Id: ugen_Buffer.cpp 1005 2010-03-16 23:10:34Z mgrobins $
// $HeadURL: http://dsrowlan@164.11.131.73/svn/CEMS/mt/other/UGenProject/UGen/buffers/ugen_Buffer.cpp $
	...
	allocatedSize(size)
{
	...
	if(size > sourceDataSize)
	{
		...
		memset(data + size, 0, (size - sourceDataSize) * sizeof(float));
	}
	else
	{
...
Buffer Buffer::copy() const throw()
{
	...
	if(size_ > 0 && numChannels_ > 0) 
	{
		for(int i = 0; i < numChannels_; i++) 
		{
			//memcpy(newBuffer.getData(i), getDataReadOnly(i), size_ * sizeof(float));
						 ...
						 const int endChannel) const throw()
{
	...
	{
		//const float *inputSamples = getDataReadOnly(currentChannel) + startSample;
...
Buffer Buffer::rand(const int size, const double lower, const double upper, const int numChannels) throw()
{
	...
	for(int channel = 0; channel < numChannels; channel++)
	{
		...
		{
			newBuffer.setSampleUnchecked(channel, sample, (float)(random.nextLinearDouble(range) + lower));
		}
	}
