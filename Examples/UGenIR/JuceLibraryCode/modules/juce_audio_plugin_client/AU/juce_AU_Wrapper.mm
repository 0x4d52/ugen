...
public:
          ...
          prepared (false)
    {
        ...
        juceFilter = static_cast<UGenPlugin*> (createPluginFilter());
                                      ...
                                      AudioUnitParameterInfo& outParameterInfo)
    {
             ...
             && index < juceFilter->getNumParameters())
        {
            ...
            //// UGenAUPatch - update the following lines ////
            outParameterInfo.minValue = juceFilter->getParameterMin(index);
            outParameterInfo.maxValue = juceFilter->getParameterMax(index);
            outParameterInfo.defaultValue = juceFilter->getParameterNominal(index);
			
			String units = juceFilter->getParameterUnits(index).trim();
			
			if(units.trim().equalsIgnoreCase("hz") || units.trim().equalsIgnoreCase("cps"))
			{
				outParameterInfo.unit = kAudioUnitParameterUnit_Hertz;
			}
			else if(units.trim().equalsIgnoreCase("x"))
			{
				outParameterInfo.unit = kAudioUnitParameterUnit_Ratio;
			}
			else if(units.trim().equalsIgnoreCase("db"))
			{
                                  ...
                                  Float32& outValue)
    {
        ...
        {
        	// UGen AU Patch
            outValue = juceFilter->getMappedParameter ((int) inID);
            return noErr;
        }
                                  ...
                                  UInt32 inBufferOffsetInFrames)
    {
        ...
        {
            // UGen AU Patch
            juceFilter->setMappedParameter ((int) inID, inValue);
            return noErr;
        }
...
private:
    //==============================================================================
    ScopedPointer<UGenPlugin> juceFilter;
